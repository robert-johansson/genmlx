...
3. **API freeze**: the env/agent/solver/belief surface is frozen as **tests-as-contracts** — not formal protocols but the per-constructor return-map shapes and the family-specific `:act`/belief signatures, documented in `src/genmlx/agents/CONTRACTS.md` and pinned by `agents_contracts_test` / `agents_api_test` / `belief_tensor_test` (the `genmlx-youg` "promotion" criterion). The minimal guaranteed contract is `{:act :params}`; everything else is family-specific (MDP / biased / POMDP / bandit), by design. `genmlx.agents.remote` is explicitly fenced PROVISIONAL, outside the frozen surface. (M)
