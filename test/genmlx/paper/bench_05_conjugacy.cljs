(ns genmlx.paper.bench-05-conjugacy
  "Paper Experiment 5: L3 Auto-Conjugacy Showcase.
   ...
   Usage: bun run --bun nbb test/genmlx/paper/bench_05_conjugacy.cljs"
...
(def results-dir
  (.resolve path-mod (js/process.cwd) "results/paper/exp05_conjugacy"))
...
(defn run-trials
  ...
  (let [trials (mapv (fn [t] (run-is-trial model args obs n-particles (+ seed-base t)))
                     (range n-trials))
...
(defn run-sub-experiment
  ...
  (let [model-ak (dyn/auto-key model)
        ...
        l3-timing (benchmark (str label "-L3")
                             ...
                             :warmup-n 5 :outer-n 5 :inner-n 10)
...
(write-json "conjugacy_results.json"
  {:experiment "exp05_conjugacy"
