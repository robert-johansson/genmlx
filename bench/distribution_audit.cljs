(ns bench.distribution-audit
  "Distribution Audit — runs all distribution test suites and reports results.
   ...
   Output: $GENMLX_RESULTS_DIR/data.json or results/distribution-audit/data.json

   Usage: bun run --bun nbb bench/distribution_audit.cljs")
...
(def out-dir
  (or (aget (.-env js/process) "GENMLX_RESULTS_DIR")
      (.resolve path-mod (js/process.cwd) "results/distribution-audit")))
...
(def suites
  [{:name "Distribution properties"
    :file "test/genmlx/dist_property_test.cljs"
    :expected nil}
   {:name "Distribution normalization"
    :file "test/genmlx/dist_normalization_property_test.cljs"
    :expected nil}
   {:name "Distribution unit tests"
    :file "test/genmlx/dist_test.cljs"
    :expected nil}
   {:name "Wishart gradient verification"
    :file "test/genmlx/wishart_gradient_test.cljs"
    :expected nil}])
...
(let [t0 (perf-now)
      ...
      data {:experiment "distribution-audit"
            :description "All distributions: normalization, moments, gradient checks"
            :suites results
            :totals totals
            :coverage {:property-tests true
                       :normalization true
                       :gradients true}}]
