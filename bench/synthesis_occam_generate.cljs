(ns bench.synthesis-occam-generate
  "ONE-SHOT live-LLM generator for the synthesis-Occam experiment (genmlx-heaw).
   ...
   PROPOSER TIERS (genmlx-8dfk, paper E11): the default tier is the dense
   Qwen3-0.6B base (consumer-hardware story). GENMLX_OCCAM_MODEL +
   GENMLX_OCCAM_TIER re-run the same tasks/seeds against a stronger proposer
   (Qwen3-Coder-Next), freezing to a TIER-SUFFIXED fixture next to the 0.6b
   one — both tiers stay committed, the scorer reports both.

   Run: bun run --bun nbb bench/synthesis_occam_generate.cljs
        GENMLX_OCCAM_MODEL=<dir> GENMLX_OCCAM_TIER=coder-next \\
          bunx --bun nbb@1.4.208 bench/synthesis_occam_generate.cljs"
...
(def tier
  "Proposer tier label; \"qwen3-0.6b\" (default) writes the original fixture,
   any other value writes synthesis_occam_programs_<tier>.edn beside it."
  (or (aget (.-env js/process) "GENMLX_OCCAM_TIER") "qwen3-0.6b"))

(def model-path
  (or (aget (.-env js/process) "GENMLX_OCCAM_MODEL")
      (str (.-HOME js/process.env) "/.cache/models/qwen3-0.6b-mlx-bf16")))

(def fixture-path
  (.resolve path-mod (js/process.cwd)
            (if (= tier "qwen3-0.6b")
              "bench/fixtures/synthesis_occam_programs.edn"
              (str "bench/fixtures/synthesis_occam_programs_"
                   (str/replace tier #"[^a-zA-Z0-9]" "_") ".edn"))))
...
(defn- git-sha []
  (try (-> (.execSync (js/require "child_process") "git rev-parse --short HEAD")
           (.toString) (str/trim))
...
(defn write-fixture! [records]
  ...
  (.writeFileSync fs fixture-path
                  (str ";; Frozen LLM-authored programs for the synthesis-Occam experiment "
                       "(genmlx-heaw; tiers genmlx-8dfk).\n"
                       ";; Generated by bench/synthesis_occam_generate.cljs — tier " tier
                       " (knowledge mode).\n"
                       ";; model: " model-path "\n"
                       ";; git: " (git-sha) "  date: " (.toISOString (js/Date.)) "\n"
